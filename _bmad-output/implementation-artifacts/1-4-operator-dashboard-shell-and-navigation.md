...
1. **Desktop sidebar (≥1024px):** Authenticated operator sees a fixed left sidebar with a warm gradient background (`linear-gradient(180deg, #C45A2D, #2E3E50)`), 240px wide, collapsible to 64px icon-only. Contains navigation items: Dashboard, Listings, Bookings, Messages, Settings. White text. Everything.Rent logo/wordmark at top. Collapse toggle control persists collapse state across navigations within the session.
