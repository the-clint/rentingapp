...
2. `headers()` — read `x-forwarded-proto` + `host`. This covers most proxies (Netlify, etc.).
...
- Detail page Server Component builds `bookingUrl` via `buildBookingUrl` helper: `NEXT_PUBLIC_SITE_URL` > `x-forwarded-proto` + `host` headers > `http://everything.test` fallback.
